...
void main() {
  group('RemoveBookmarkFromVaultUseCase', () {
    late _FakeBookmarkItemRepository fakeRepository;
    late RemoveBookmarkFromVaultUseCase useCase;

    setUp(() {
      fakeRepository = _FakeBookmarkItemRepository();
      useCase = RemoveBookmarkFromVaultUseCase(repository: fakeRepository);
    });

    test('returns true and sets isInVault=false when bookmark exists',
        () async {
      ...
      expect(updated!.isInVault, isFalse);
    });
    ...
    test('is idempotent — already out of vault still updates timestamp',
        () async {
      final DateTime originalUpdatedAt = DateTime(2024, 1, 1, 12);
      final BookmarkItem bookmark = BookmarkItem(
        originalUrl: 'https://example.com/secret',
